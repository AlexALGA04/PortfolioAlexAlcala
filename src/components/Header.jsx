import React from 'react'
import '../App.css'
import githubIcon  from '../images/icons/github_icon.png'
import linkedinIcon from '../images/icons/linkedin_icon.png'
import gmailIcon from '../images/icons/gmail_icon.png'

function Header() {
  return (
    <header className="lg:flex">
        <nav className="w-fit mx-auto lg:mx-0">
            <ul className='flex text-white justify-center border-1 border-[#A855F7] p-3 px-5 rounded-full lg:w-fit lg:px-10'>
              <li className='mr-6 lg:mr-10 lg:text-[20px]'><a href="#inicio">Inicio</a></li>
              <li className='mr-6 lg:mr-10 lg:text-[20px]'><a href="#habilidades">Habilidades</a></li>
              <li className='mr-6 lg:mr-10 lg:text-[20px]'><a href="#proyectos">Proyectos</a></li>
              <li className="lg:text-[20px]"><a href="#estudios">Estudios</a></li>
            </ul>
        </nav>
        <div className='hidden ml-auto px-10 text-white border-1 border-[#A855F7] p-3 rounded-full lg:flex lg:w-fit lg:px-10'>
            <a className='mr-10' href="https://github.com/AlexALGA04" target="_blank">
            <img className='w-[30px] h-[30px] object-contain' src={githubIcon} alt="GitHub"/>
            </a>
            <a className='mr-10' href="https://linkedin.com/in/tuusuario" target="_blank">
            <img className='w-[30px] h-[30px] object-contain' src={linkedinIcon} alt="LinkedIn"/>
            </a>
            <a href="https://linkedin.com/in/tuusuario" target="_blank">
            <img className='w-[30px] h-[30px] object-contain' src={gmailIcon} alt="SendGmail"/>
            </a>
        </div>
    </header>
  )
}

export default Header